...
set.seed(1859)
M_true_mean <- 50
weights <- rnorm(300, mean = M_true_mean, sd = 3)
...
We **assume we KNOW the standard deviation**. In this case, it is exactly 3. 
...
propose_new_val <- function(old_value, tune) {
...
for (i in 2:length(chain)) {
  ...
  new_val <- propose_new_val(old_value = old_val, tune = tune_parameter)
...
plot(rejects)
# 
n_rejects <- sum(rejects == 0)  / length(chain)
```
...
In this case, we need to adjust our calculation of the probability of acceptance. Now in addition to the Likelihood and Prior, we have the Candidate distribution getting involved
...
That means we can't directly set the mean and variance of a distribution with one parameter each.  

<aside>except for the Normal distribution. The Normal is a beautiful weirdo which is why we love it!</aside>
...
tibble(v = chain[300:n_samples]) %>% 
  ...
  geom_line(aes(x = sigma, y = density), 
            data = tibble(sigma = seq(from = 2.6, to = 3.3, length.out = n_samples - 300), 
