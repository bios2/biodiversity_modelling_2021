...
See the [slides on this part here](https://bios2.github.io/biodiversity_modelling_2021/slides/12_Bayes#49)

See [The slides on the topic]

The same, but we can use an asymmetrical proposal distribution.  This means that we can jump in either direction, but one direction is more likely

We need this, because sometimes our parameters are bounded! For example, the standard deviation of a normal distribution can never be negative. 

Because our jumps are no longer symmetrical, we need to add a small correction factor 
...
Let's go (back) to our normal example and try this out.

**We have to assume we KNOW the mean**. Later we'll see examples of what to do if you want to find two or more numbers at once! 

```{r}

weights <- rnorm(30, mean = 50, sd = 3)
...
That means we can't directly set the mean and variance of a distribution with one parameter each. <aside> except for the Normal distribution. The Normal is a beautiful weirdo which is why we love it </aside>.
...
## define likelihoods for our problem
normal_loglike <- function(data_vec, sd_value){
  sum(dnorm(data_vec, mean = 50, sd = sd_value, log = TRUE))
}

gamma_loglike <- function(sd_value){
  dgamma(sd_value, shape = 4^2/3^2, rate = 4/3^2, log = TRUE)
}
...
# proposal function to make new parameter values.
propose_new_val <- function(old_value, sig_tune) {
  rlnorm(1, mean = log(old_value), sd = sig_tune)
}

propose_density <- function(jump_to, jump_from, sig_tune) {
...
numerator <- function(v, data)  {
  # likelihood
  log_like <- normal_loglike(data_vec = data, sd_value = v)
  # prior
  log_prior <- gamma_loglike(sd_value = v)
  
  return(log_like + log_prior)
}

## function to calculate the acceptance ratio
accept_probability <- function(new_value, old_value, 
                               data_to_use, 
                               tune_parameter) {
  
  # working on the LOG scale so we SUBTRACT values (the same as dividing these two things)
  new <- numerator(v = new_value, data = data_to_use)
  old <- numerator(v = old_value, data = data_to_use)
  ...
  new_from_old <- propose_density(jump_to = new_value,
                                  jump_from = old_value,
                                  sig_tune = tune_parameter)
  
  old_from_new <- propose_density(jump_to = old_value,
                                  jump_from = new_value,
                                  sig_tune = tune_parameter)
...
The proposal distribution, centered on a value (e.g. 9) should be the same as the proposal density, centered on the same value.
```{r}

replicate(300, propose_new_val(9, 0.1)) %>% hist(probability = TRUE)
curve(propose_density(jump_to = x, jump_from = 9, sig_tune = 0.1), xlim = c(5,14), add = TRUE)
...
set.seed(1859)
correct_sd <- 3
weights <- rnorm(1000, mean = 50, sd = correct_sd)
...
for (i in 2:length(chain)) {
  ...
  new_val <- propose_new_val(old_value = old_val, sig_tune = tune_parameter)
  
  p_accept <- accept_probability(new_value = new_val,
                                 old_value = old_val, 
                                 data_to_use = weights,
                                 tune_parameter = tune_parameter)
  
  chain[i] <- ifelse(runif(1) < p_accept, new_val, old_val)
}



plot(chain[300:n_samples], type = "l")
```

```{r}

plot(density(chain[300:n_samples]))
abline(v = correct_sd)
```

```{r}
median(chain[300:n_samples])
mean(chain[300:n_samples])
```
...
Now instead of setting the prior on $\sigma$ directly, we set the prior on the _inverse square_ of sigma! <aside>Why do we do the inverse square? Well, the square of the standard deviation is the variance. As we know, the larger the variance, the wider the distribution. The inverse variance has the opposite interpretation: the larger it is, the more narrow and focussed the distribution! This is called the "precision", and it is another common way to think about the Normal Distribution.</aside> 
...
```{r}
curve(dgamma(x, 3.5^2/3^2 + length(weights)/2, 3.5/3^2 + sum((weights - 50)^2)/2))
```
...
plot(x = precision_values^-.5, y = post_density)

(precision_values^-.5)[which.max(post_density)]
abline(v = correct_sd)
```

Compare to the Metropolis-Hastings approach

```{r}
plot(x = precision_values^-.5, y = post_density, xlim = c(2.7, 3.4))
hist(chain[300:n_samples], probability = TRUE, add= TRUE)
lines(density(chain[300:n_samples]))
```
...
```{r}
data_frame(v = chain[300:n_samples]) %>% 
  ggplot(aes(x = v)) + 
  geom_histogram(aes(y = ..density..),
                 binwidth = .01, fill = "grey", color = "black") + 
  geom_line(aes(x = sigma, y = density), data.frame(sigma = precision_values^-.5, 
                                                     density = post_density*6/80))
```
...
data_frame(v = chain[300:n_samples]) %>% 
  ...
  geom_line(aes(x = sigma, y = density), 
            data = data_frame(sigma = seq(from = 2.6, to = 3.3, length.out = n_samples - 300), 
                              density = dgamma(sigma^(-2),
                                               shape = 2^2/5^2 + length(weights)/2,
                                               rate = 2/5^2 + sum((weights - 50)^2)/2)))
```


Calculate the posterior average
...
prec_mean <- (.02^2/5^2 + length(weights)/2)/(.02/5^2 + sum((weights - 50)^2)/2)
prec_mean^-.5
```
...
```{r}
pp <- replicate(200, {
weights <- rnorm(1e5, mean = 50, sd = correct_sd)
